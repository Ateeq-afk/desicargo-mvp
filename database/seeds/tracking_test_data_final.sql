-- Tracking System Test Data (Final)
...
BEGIN
    ...
    IF test_consignment_id IS NULL THEN
        INSERT INTO consignments (
            ...
            freight_amount, total_amount, payment_mode,
            status, created_by, from_city, to_city
        ) VALUES (
            ...
            500.00, 590.00, 'paid',
            'BOOKED', admin_user_id, 'Delhi', 'Mumbai'
        ) RETURNING id INTO test_consignment_id;
    END IF;
    
    RAISE NOTICE 'Using consignment ID: %', test_consignment_id;
    
    -- Clear existing tracking events for this consignment
    DELETE FROM tracking_events WHERE consignment_id = test_consignment_id;
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM tracking_events te WHERE te.consignment_id = c.id) as tracking_events
    FROM consignments c 
    WHERE c.id = test_consignment_id;
